	  ...
	  if(mem_h != (transfer->buffer[(i*944)+1]&0x01))   
	      ...
	      for(k=0;k<(int)desc->actual_length;k++)
		{
		  if(k>11)
	       ...
	       void* buffer_ptr= (void *)transfer->buffer;
               memcpy((void *) (image_buffer + offset), (const void *)(buffer_ptr +((i*944)+12)),(desc->actual_length)-12);
	       offset=offset + desc->actual_length -12; 
	    }
	  else
	    {
...
/*We did this by referring the 'uvc_set_video_ctrl' function defined in the uvc_video.c file of the Linux UVC Driver found in Linux Source Code*/
  ...
  *(__le16 *)&data[0] = (uint16_t)(0x01); //Supposed to be value of bmHint, unknown to us
  data[2] = 0x01; //Uncompressed Format
  data[3] = 0x04; //320x240
...
*/
  ...
  //The values to be sent in each of these packets were found from the USB_Video_Example 1.5.pdf from the spec.
  //Refer the sections starting from 2.4.2.1.1 on page number 26 of that pdf.
...
/*The GET_CUR request will return some new data, which will help us selecting the correct alternate setting*/
...
void* vpipe_client(void *ptr)           /*Send images to the host for display and debug over a TCP socket*/
{
        FILE *fp;
